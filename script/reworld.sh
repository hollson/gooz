...
shell_path=$(cd $(dirname $0);pwd) #当前脚本路径
...
function install() {
    ...
    sleep 1s
     ...
     # 安装程序
     cur=$(pwd)
     sudo mkdir -p $install_path || { return ; exit 0; } &&
     sudo /bin/cp -rf $cur/* $install_path/ || { return ; exit 0; } &&
...
function update() {
    ...
    # 安装程序
    sudo /bin/cp -rf ./release/${REWORLD_CONF:4}/* $install_path/ || { return ; exit 0; }
    cd -
